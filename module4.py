...
class UIPrompt:
    ...
    def init_prompt(self, nuser, nitem, codebook_hobby_k, codebook_background_k):
        ...
        emsize = self.model.config.hidden_size  # 768维度，soft embedding 的维度,相当于LLM中要用的固定维度大小. 用model.transformer.wte可以看出
    ...
    def forward(self, user, item, text, mask, seq_price, mask_price, seq_brand, mask_brand, ignore_index=-100):
        ...
        w_src = self.model.embed_tokens(text).to(device)  # (batch_size, tgt_len, emsize) wte相当于词向量的东西输入词id进去得到词向量 word token embedding;
        w_hb_src = self.model.embed_tokens(seq_brand).to(device)
        w_bg_src = self.model.embed_tokens(seq_price).to(device)
...
class DiscretePromptLearning(FeaturePrompt, LlamaForCausalLM):
    def __init__(self, config):
        super().__init__(config)
...
class RecReg(UIPromptWithReg, LlamaForCausalLM):
    def __init__(self, config):
        super().__init__(config)
