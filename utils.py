...
class DataLoader:
    def __init__(self, data_path, index_dir, tokenizer, seq_len):
        self.user_dict = EntityDictionary() # 上面定义的class EntityDictionary，
    ...
    def load_data(self, data_path, index_dir):
        ...
        for review in reviews: # reviews为列表，每条review为一个用户评论，
            ...
            tokens = self.tokenizer(tem)['input_ids'] # ！！把一句话转换为每个单词的id编号，
...
class Batchify:
    def __init__(self, data, tokenizer, bos, eos, batch_size=128, shuffle=False):
        u, i, r, t, self.feature = [], [], [], [], [] # r为u对i的评分；t为加了开头bos结尾eos的句子
