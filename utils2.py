    ...
    def load_data(self, data_path, index_dir):
        ...
        for review in reviews: # reviews为列表，每条review为一个用户评论，
            ...
            data.append({'user': self.user_dict.entity2idx[review['user']],
                         ...
                         'price': self.price_dict[review['item']],
                         'brand': self.brand_dict[review['item']], 
...
class Batchify:
    def __init__(self, data, tokenizer, bos, eos, batch_size=128, shuffle=False):
        ...
        for x in data: # 一般data代的就是args.train训练集数据，
            u.append(x['user']) 
            i.append(x['item'])
            r.append(x['rating'])
            ...
            price.append('{} {} {}'.format(bos, x['price'], eos)) # 价格是float浮点数
            brand.append('{} {} {}'.format(bos, x['brand'], eos)) # brand是文本数据
        ...
        encoded_inputs_brand = tokenizer(brand, padding=True, return_tensors='pt') # 这一步是转为id了，
        ...
        encoded_inputs_price = tokenizer(price, padding=True, return_tensors='pt') # 这一步是转为id了，
